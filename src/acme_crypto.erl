...
-spec create_jws_header(string(), public_key:private_key(), string() | undefined, string()) ->
    map().
...
-doc """
Converts an RSA private key to JWK (JSON Web Key) format.
...
-doc """
Computes the JWK thumbprint for an RSA private key.
...
-doc """
Generates the DNS TXT record value from key authorization.
...
base64url_encode(Data) when is_binary(Data) ->
    ...
    % Convert to URL-safe base64 and ensure it's a flat string
    EncodedStr = binary_to_list(Encoded),
    % Use re:replace for more reliable string replacement
    NoPlus = re:replace(EncodedStr, "\\+", "-", [global, {return, list}]),
    NoSlash = re:replace(NoPlus, "/", "_", [global, {return, list}]),
    re:replace(NoSlash, "=", "", [global, {return, list}]);
base64url_encode(Data) when is_list(Data) ->
    base64url_encode(list_to_binary(Data)).
...
base64url_decode(Data) when is_list(Data) ->
    ...
    WithPlus = re:replace(Data, "-", "+", [global, {return, list}]),
    WithSlash = re:replace(WithPlus, "_", "/", [global, {return, list}]),
    % Add padding if necessary
    PaddedLength = 4 * ((length(WithSlash) + 3) div 4),
    Padding = lists:duplicate(PaddedLength - length(WithSlash), $=),
    Padded = WithSlash ++ Padding,
    base64:decode(list_to_binary(Padded)).

-doc """
Creates a JWS header for ACME requests.
...
-doc """
Creates a JWS signature for the given header and payload.
...
-doc """
Signs data with the specified algorithm and private key.
