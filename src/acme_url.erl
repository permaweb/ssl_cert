...
extract_base_url(Url) ->
    ...
    case string:split(UrlStr, "://") of
        [Scheme, Rest] ->
            [Host | _] = string:split(Rest, "/"),
            ssl_utils:list(Scheme) ++ "://" ++ ssl_utils:list(Host);
        [_] ->
            % No scheme, assume https
            [Host | _] = string:split(UrlStr, "/"),
            "https://" ++ ssl_utils:list(Host)
    end.
...
extract_host_from_url(Url) ->
    ...
    case string:split(UrlStr, "://") of
        [_Scheme, Rest] ->
            [Host | _] = string:split(Rest, "/"),
            ssl_utils:bin(ssl_utils:list(Host));
        [Host] ->
            [HostOnly | _] = string:split(Host, "/"),
            ssl_utils:bin(ssl_utils:list(HostOnly))
    end.
...
extract_path_from_url(Url) ->
    ...
    case string:split(UrlStr, "://") of
        [_Scheme, Rest] ->
            [_Host | PathParts] = string:split(Rest, "/"),
            "/" ++ string:join([ssl_utils:list(P) || P <- PathParts], "/");
        [Rest] ->
            [_Host | PathParts] = string:split(Rest, "/"),
            "/" ++ string:join([ssl_utils:list(P) || P <- PathParts], "/")
    end.
...
-doc """
Determines the ACME directory URL from an account record.
...
-doc """
Converts header list to map format.
...
-doc """
Normalizes a URL to a consistent string format.
...
normalize_url(Url) ->
    ...
    case string:prefix(UrlStr, "http://") of
        nomatch ->
