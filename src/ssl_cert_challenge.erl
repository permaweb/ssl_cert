...
-moduledoc """
SSL Certificate challenge management module.
...
%% Helper exports
-export([
    all_challenges_valid/1,
    merge_validation_results/2,
    results_to_domain_map/1,
    extract_domain_bin/1
]).
...
-spec validate_challenges_with_timeout(acme_account(), [map()], integer()) ->
    [validation_result()].
-spec poll_challenge_status(
    acme_account(), dns_challenge(), string(), integer(), integer(), integer()
) ->
    validation_result().
...
-spec all_challenges_valid([validation_result()]) -> boolean().
-spec merge_validation_results([validation_result()], [validation_result()]) ->
    [validation_result()].
-spec results_to_domain_map([validation_result()]) -> map().
-spec extract_domain_bin(map() | dns_challenge()) -> binary().

-doc """
Validates DNS challenges and manages the complete validation workflow.
...
validate_dns_challenges_state(State, RSAPrivKey) ->
    case State of
        State when is_map(State) ->
            ...
            AllValid = all_challenges_valid(ValidationResults),
            case AllValid of
                true ->
                ...
                false ->
                    ...
                    handle_some_challenges_failed(
                        State, Account, Challenges, ValidationResults, RSAPrivKey
                    )
            end;
        _ ->
...
-doc """
Validates DNS challenges with Let's Encrypt with polling and timeout.
            ...
            case acme_client:validate_challenge(Account, ChallengeRecord) of
                {ok, InitialStatus} ->
                    ...
                    poll_challenge_status(
                        Account, ChallengeRecord, Domain, StartTime, TimeoutSeconds, 1
                    );
                {error, Reason} ->
                    ...
                    #{
                        ...
                        <<"status">> => <<"failed">>,
                        <<"error">> => ssl_utils:bin(
                            io_lib:format("Failed to trigger validation: ~p", [Reason])
                        )
                    }
            end
        end,
...
-doc """
Polls a challenge status until it reaches a final state or times out.
                ...
                {ok, Status} ->
                    ...
                    case StatusBin of
                        ?ACME_STATUS_VALID ->
                            ?event(
                                ssl_cert,
                                {ssl_cert_challenge_validation_success, Domain, AttemptNum}
                            ),
                            #{
                                <<"domain">> => ssl_utils:bin(Domain),
                        ...
                        ?ACME_STATUS_INVALID ->
                            ?event(
                                ssl_cert, {ssl_cert_challenge_validation_failed, Domain, AttemptNum}
                            ),
                            #{
                                <<"domain">> => ssl_utils:bin(Domain),
                        ...
                        _ when
                            StatusBin =:= ?ACME_STATUS_PENDING;
                            StatusBin =:= ?ACME_STATUS_PROCESSING
                        ->
                        ...
                        _ ->
                            ...
                            #{
                                ...
                                <<"status">> => StatusBin,
                                <<"error">> => ssl_utils:bin(
                                    io_lib:format("Unknown status: ~s", [Status])
                                ),
                                <<"attempts">> => AttemptNum
                            }
                    end;
...
-doc """
Formats challenges for user-friendly HTTP response.
    ...
    lists:map(
        fun(Challenge) ->
            ...
            #{
                ...
                <<"instructions">> => #{
                    <<"cloudflare">> => ssl_utils:bin(
                        "Add TXT record: _acme-challenge with value " ++ DnsValue
                    ),
                    <<"route53">> => ssl_utils:bin(
                        "Create TXT record " ++ RecordName ++ " with value " ++ DnsValue
                    ),
                    <<"manual">> => ssl_utils:bin(
                        "Create DNS TXT record for " ++ RecordName ++ " with value " ++ DnsValue
                    )
                }
            }
        end,
...
-doc """
Handles the case where all challenges are valid.
        ...
        _ ->
            ...
            case acme_client:finalize_order(Account, Order, RSAPrivKey) of
                {ok, FinalizedOrder} ->
                    ...
                    PollResult = poll_order_until_valid(
                        Account, UpdatedState, ?ORDER_POLL_TIMEOUT_SECONDS
                    ),
                    case PollResult of
                        {valid, PolledState} ->
                            {ok, #{
                        ...
                        {error, PollReason} ->
                            {error, #{
                                <<"status">> => 500,
                                <<"error">> => ssl_utils:bin(
                                    io_lib:format("Order polling failed: ~p", [PollReason])
                                )
                            }}
                    end;
                {error, FinalizeReason} ->
                    ...
                    {ok, #{
                        ...
                        <<"body">> => #{
                            <<"message">> =>
                                <<"DNS challenges validated but finalization pending">>,
                            <<"results">> => ValidationResults,
                            <<"order_status">> => ?ACME_STATUS_PROCESSING,
                            <<"request_state">> => State,
...
-doc """
Handles the case where some challenges failed.
...
handle_some_challenges_failed(State, Account, Challenges, ValidationResults, RSAPrivKey) ->
    ...
    RetryChallenges = lists:filter(
        fun(C) -> not lists:member(extract_domain_bin(C), ValidDomains) end,
        Challenges
    ),
    case RetryChallenges of
        ...
        _ ->
            ...
            MergedResults = merge_validation_results(ValidationResults, RetryResults),
            AllValidAfterRetry = all_challenges_valid(MergedResults),
            case AllValidAfterRetry of
                ...
                false ->
                    {ok, #{
                        ...
                        <<"body">> => #{
                            <<"message">> =>
                                <<"DNS challenges validation completed with some failures ",
                                    "(retry attempted)">>,
...
-doc """
Extracts challenge information from various challenge formats.
...
-doc """
Internal loop for polling order status.
    ...
    case erlang:system_time(second) - Start < TimeoutSeconds of
        ...
        true ->
            case acme_client:get_order(Account, OrderUrl) of
                {ok, Resp} ->
                    StatusBin = ssl_utils:bin(
                        maps:get(<<"status">>, Resp, ?ACME_STATUS_PROCESSING)
                    ),
                    ...
                    UpdatedState = State#{
                        <<"order">> => UpdatedOrderMap, <<"status">> => StatusBin
                    },
                    case StatusBin of
                        ?ACME_STATUS_VALID ->
                            {valid, UpdatedState};
                        _ ->
                            timer:sleep(?ORDER_POLL_DELAY_SECONDS * 1000),
                            poll_order_until_valid_loop(
                                Account, UpdatedState, TimeoutSeconds, Start
                            )
                    end;
                {error, Reason} ->
                    {error, Reason}
...
-doc """
Extracts domain as binary from challenge record or map.
...
@returns Domain as binary string
""".
extract_domain_bin(Challenge) ->
    case Challenge of
        #{<<"domain">> := D} -> D;
...
-doc """
Merges validation results by domain, with retry results taking precedence.

@param OriginalResults Original validation results
@param RetryResults Retry validation results
@returns Merged list of validation results
""".
merge_validation_results(OriginalResults, RetryResults) ->
    OrigMap = results_to_domain_map(OriginalResults),
    RetryMap = results_to_domain_map(RetryResults),
    MergedMap = maps:merge(OrigMap, RetryMap),
    [V || {_K, V} <- maps:to_list(MergedMap)].

-doc """
Converts validation results list to a domain-keyed map.

@param Results List of validation result maps
@returns Map with domain as key and result as value
""".
results_to_domain_map(Results) ->
    maps:from_list([{maps:get(<<"domain">>, R), R} || R <- Results]).
