-module(acme_csr).
-moduledoc """
ACME Certificate Signing Request (CSR) generation module.
...
%% Utility exports
-export([
    normalize_asn1_params/1,
    create_extension_request_attribute/1,
    is_ip_address/1,
...
-spec generate_csr_internal([string()], map()) ->
    {ok, binary(), public_key:private_key()} | {error, term()}.
...
-spec create_complete_rsa_key_from_wallet(integer(), integer(), integer()) ->
    public_key:rsa_private_key().

%% Utility specifications
-spec normalize_asn1_params(term()) -> term().
-spec create_extension_request_attribute(term()) -> term().
-spec is_ip_address(term()) -> boolean().
-spec ip_address_to_binary(term()) -> binary().
-spec validate_single_domain(term()) -> {ok, term()} | {error, term()}.
-spec validate_all_domains(term()) -> {ok, [term()]} | {error, term()}.

-doc """
Generates a Certificate Signing Request for the specified domains.
...
-doc """
Internal CSR generation with comprehensive error handling.
...
generate_csr_internal(Domains0, RSAPrivKey) ->
    try
        ...
        case validate_domains(Domains0) of
            {ok, Domains} ->
                % First domain becomes Common Name
                CN = hd(Domains),
...
-doc """
Internal function to generate CSR with validated domains.
""".
generate_csr_with_domains(CN, Domains, RSAPrivKey) ->
...
-doc """
Creates the certificate subject with Common Name.
...
-doc """
Creates a Subject Alternative Name extension for multiple domains.
...
-doc """
Validates and normalizes a list of domain names.
    ...
    try
        ...
        case Domains of
            ...
            _ ->
                ...
                case validate_all_domains(Domains) of
                    {ok, ValidatedDomains} ->
                        {ok, ValidatedDomains};
                    {error, Reason} ->
                        error(Reason)
        ...
        Error:CatchReason ->
            {error, {domain_validation_failed, Error, CatchReason}}
    end.

-doc """
Normalizes a domain name to binary format.
...
-doc """
Normalizes ASN.1 algorithm parameters for compatibility.
...
-doc """
Creates an extension request attribute for CSR.
...
-doc """
Checks if a domain string represents an IP address.
...
@returns {ok, ValidatedDomain} or {error, {invalid_domain, Reason}}
""".
validate_single_domain(Domain) ->
    % Basic domain validation - could be enhanced with more checks
    case byte_size(Domain) of
        0 -> {error, {invalid_domain, empty_domain}};
        Size when Size > 253 -> {error, {invalid_domain, domain_too_long}};
        _ -> {ok, Domain}
    end.

-doc """
Validates all domains in a list.
...
@returns {ok, ValidatedDomains} or {error, Reason}
""".
validate_all_domains(Domains) ->
    Results = [validate_single_domain(D) || D <- Domains],
    case
        lists:partition(
            fun
                ({ok, _}) -> true;
    ...
    of
        {OkResults, []} ->
            ValidatedDomains = [D || {ok, D} <- OkResults],
            {ok, ValidatedDomains};
        {_, [{error, Reason} | _]} ->
            {error, Reason}
    end.
...
-doc """
Creates a complete RSA private key from wallet components.
