...
request_certificate(Account, Domains) ->
    ...
    try
        ...
        case acme_http:make_jws_request(NewOrderUrl, Payload, 
    ...
    try
        ...
        Challenges = lists:foldl(fun(AuthzUrl, Acc) ->
            ...
            case get_authorization(AuthzUrlStr) of
                {ok, Authz} ->
                    Domain = ssl_utils:list(maps:get(<<"value">>, 
                    ...
                    case find_dns_challenge(maps:get(<<"challenges">>, Authz)) of
                        {ok, Challenge} ->
                            ...
                            KeyAuth = acme_crypto:generate_key_authorization(Token, 
                                                               Account#acme_account.key),
...
validate_challenge(Account, Challenge) ->
    ...
    try
        Payload = #{},
        case acme_http:make_jws_request(Challenge#dns_challenge.url, Payload, 
...
get_challenge_status(Account, Challenge) ->
    ...
    try
        case acme_http:make_jws_post_as_get_request(Url, Account#acme_account.key, Account#acme_account.kid) of
            {ok, Response, _Headers} ->
                Status = ssl_utils:list(maps:get(<<"status">>, Response)),
...
finalize_order(Account, Order, RSAPrivKey) ->
    ...
    try
        ...
        Domains = [ssl_utils:list(maps:get(<<"value">>, Id)) 
        ...
        case acme_csr:generate_csr(Domains, RSAPrivKey) of
            {ok, CsrDer} ->
                ...
                case acme_http:make_jws_request(Order#acme_order.finalize, Payload,
                                     Account#acme_account.key, 
                                     Account#acme_account.kid) of
                    {ok, Response, _Headers} ->
...
get_order(Account, OrderUrl) ->
    ...
    try
        case acme_http:make_jws_post_as_get_request(OrderUrl, Account#acme_account.key, Account#acme_account.kid) of
            {ok, Response, _Headers} ->
